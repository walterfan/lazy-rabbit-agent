...
class OpenAICompatibleProvider(LLMProvider):
    ...
    async def generate_completion(
        ...
        **kwargs: Any
        ...
        try:
            # Log request details
            logger.info(f"🤖 LLM Request - Model: {self.model}, Response Type: {response_model.__name__}, Max Retries: {max_retries}")
    ...
    async def generate_completion_stream(
        ...
        **kwargs: Any
        ...
        try:
            ...
            logger.info(f"✅ LLM Streaming Complete - Tokens: {token_count}, Length: {len(complete_text)} chars")
            logger.debug(f"📦 LLM Streaming Response:\n{'-' * 80}\n{complete_text}\n{'-' * 80}")
